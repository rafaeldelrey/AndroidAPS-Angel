...
class AngelFragment : DaggerFragment() {
    ...
    fun updateGUI() {
        ...
        val AngelPlugin = activePlugin.activeAPS
        AngelPlugin.lastAPSResult?.let { lastAPSResult ->
            binding.result.text = jsonFormatter.format(lastAPSResult.json)
            binding.request.text = lastAPSResult.toSpanned()
        }
        AngelPlugin.lastDetermineBasalAdapter?.let { determineBasalAdapterAngelJS ->
            binding.glucosestatus.text = jsonFormatter.format(determineBasalAdapterAngelJS.glucoseStatusParam)
            binding.currenttemp.text = jsonFormatter.format(determineBasalAdapterAngelJS.currentTempParam)
            try {
                val iobArray = JSONArray(determineBasalAdapterAngelJS.iobDataParam)
            ...
            binding.profile.text = jsonFormatter.format(determineBasalAdapterAngelJS.profileParam)
            binding.mealdata.text = jsonFormatter.format(determineBasalAdapterAngelJS.mealDataParam)
            binding.scriptdebugdata.text = determineBasalAdapterAngelJS.scriptDebug.replace("\\s+".toRegex(), " ")
            AngelPlugin.lastAPSResult?.inputConstraints?.let {
                binding.constraints.text = it.getReasons(aapsLogger)
            }
        }
        if (AngelPlugin.lastAPSRun != 0L) {
            binding.lastrun.text = dateUtil.dateAndTimeString(AngelPlugin.lastAPSRun)
        }
        AngelPlugin.lastAutosensResult.let {
