package info.nightscout.androidaps.plugins.aps.Angel
...
@OpenForTesting
@Singleton
class AngelPlugin @Inject constructor(
    ...
    PluginDescription()
        ...
        .fragmentClass(AngelFragment::class.java.name)
    ...
    override var lastAPSResult: DetermineBasalResultAngel? = null
    ...
    override fun invoke(initiator: String, tempBasalFallback: Boolean) {
        ...
        val iobArray = iobCobCalculator.calculateIobArrayForSMB(lastAutosensResult, AngelDefaults.exercise_mode, AngelDefaults.half_basal_exercise_target, isTempTarget)
        ...
        provideDetermineBasalAdapter().also { determineBasalAdapterAngelJS ->
            determineBasalAdapterAngelJS.setData(
            ...
            val determineBasalResultAngel = determineBasalAdapterAngelJS.invoke()
            profiler.log(LTag.APS, "SMB calculation", start)
            if (determineBasalResultAngel == null) {
                ...
                if (determineBasalResultAngel.rate == 0.0 && determineBasalResultAngel.duration == 0 && iobCobCalculator.getTempBasalIncludingConvertedExtended(dateUtil.now()) == null) determineBasalResultAngel.tempBasalRequested = false
                determineBasalResultAngel.iob = iobArray[0]
                determineBasalResultAngel.json?.put("timestamp", dateUtil.toISOString(now))
                determineBasalResultAngel.inputConstraints = inputConstraints
                lastDetermineBasalAdapter = determineBasalAdapterAngelJS
                lastAPSResult = determineBasalResultAngel as DetermineBasalResultAngel
                lastAPSRun = now
            }
        }
    ...
    fun provideDetermineBasalAdapter(): DetermineBasalAdapterInterface = DetermineBasalAdapterAngelJS(ScriptReader(context), injector)
}